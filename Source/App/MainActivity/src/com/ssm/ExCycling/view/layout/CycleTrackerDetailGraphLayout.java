		...
		for(int i = 0 ; i < distList.size() ; i++){
			for(int j = 0 ; j < distList.size() ; j++){
				if(distList.get(i)<distList.get(j)){
					...
					String tempSpeed = speedList.get(j);
					speedList.set(j, speedList.get(i));
					speedList.set(i, tempSpeed);
					
					String tempAltitude = altitudeList.get(j);
					altitudeList.set(j, altitudeList.get(i));
					altitudeList.set(i, tempAltitude);
				}
			}
		}
