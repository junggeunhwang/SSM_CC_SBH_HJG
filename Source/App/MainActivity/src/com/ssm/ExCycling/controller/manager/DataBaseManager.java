...
<<<<<<< HEAD
	...
	public void insertAtmosphere(String startTime,String temp,String humidity, String wind, String wind_dir,String member_count){
		if(member_count == null)member_count="1";
		...
		db.insert("atmosphere",null,row);
	}
	
	public void selectAtmosphere(String startTime){
		db = manager.getReadableDatabase();
		Cursor c = db.rawQuery("select * from atmosphere where startTime = " + startTime + ";",null);
		c.moveToNext();
		if(c.getCount()!=0){
			MainActivity.getInstasnce().getLayout().getmFragmentCycleTracker().getCurrentActivatedData().setHumidity(c.getString(HUMIDITY));
			MainActivity.getInstasnce().getLayout().getmFragmentCycleTracker().getCurrentActivatedData().setTemp(c.getString(TEMP));
			MainActivity.getInstasnce().getLayout().getmFragmentCycleTracker().getCurrentActivatedData().setWind(c.getString(WIND));
			MainActivity.getInstasnce().getLayout().getmFragmentCycleTracker().getCurrentActivatedData().setWind_dir(c.getString(WIND_DIR));
			MainActivity.getInstasnce().getLayout().getmFragmentCycleTracker().getCurrentActivatedData().setMem_count(c.getString(MEMBER_COUNT));	
		}
